...
import { genresMapReverse } from "../utils/genres";
import Modal from "./common/Modal";
import InputField from "./ui/InputField";
import Button from "./ui/Button";
import { MovieService } from "../services/movie.service";
import type { Movie, MovieUpdatePayload } from "../types/movie.types";
...
const EditMovieForm = ({
    ...
    if (movie) {
      setFormData({
        ...
        genres:
          movie.genres
            ?.map((genreName) => {
              const genreEnum = genresMapReverse[genreName];
              const foundGenre = GENRES.find((g) => g.value === genreEnum);
              return foundGenre ? foundGenre.id : 0;
            })
            .filter(Boolean) || [],
  ...
  const handleUpdateMovie = async (): Promise<Movie> => {
    ...
    const moviePayload: MovieUpdatePayload = {
    ...
    const updatedMovie = await MovieService.updateMovie(movie.id, moviePayload);
    return updatedMovie;
  };
  ...
  const handleSkipAndSave = async () => {
    setIsSaving(true);
    try {
      const updatedMovie = await handleUpdateMovie();
      console.log(`Filme ${updatedMovie.id} atualizado sem imagens!`);
      onSave(updatedMovie);
      handleCloseAndReset();
    } catch (err: unknown) {
  ...
  const handleSubmitImages = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(`Filme ${updatedMovie.id} e imagens atualizados!`);
      onSave(updatedMovie);
      handleCloseAndReset();
    } catch (err: unknown) {
      console.error("Erro ao atualizar filme/imagens:", err);
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <div className="bg-form-background fixed right-0 top-0 bottom-0 md:w-[500px] w-full overflow-y-auto shadow-xl">
        <div className="sticky top-0 bg-form-background z-10 p-6 border-b border-button-primary-disable flex justify-between items-center">
          <h2 className="text-2xl font-bold text-white">
            {step === "info" ? "Editar Filme" : "Imagens do Filme"}
          </h2>
          <button
          ...
          {step === "info" ? (
            <form onSubmit={handleNextStep} className="space-y-4 pb-20">
              ...
              <InputField
                ...
                type="date"
                value={formData.releaseDate.toString()}
              ...
              <div className="space-y-8">
                ...
                <div className="flex justify-end space-x-2 pt-4">
                  ...
                  <Button
                    ...
                    variant="primary"
                    disabled={
                      isSaving || !formData.posterImage || !formData.bannerImage
                    }
                  >
                    Finalizar
                  </Button>
